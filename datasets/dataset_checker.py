...
ROOT_DIR = "D:/NVT/RecordingChecker/RecordingData"
MAIN_IDX = 2
SUB_IDX = 0
DIFFICULTY = "Difficult"
record_name = "AnimeFantasyRPG_0_60"
cfg_list = [
    DatasetConfig(
        record=record_name,
        main_idx=MAIN_IDX,
...
def G_Buffer(raw_df: pd.DataFrame, G_buffer_prefix: str) -> pd.DataFrame:
    invalid_count = 0
    with tqdm(range(1, len(raw_df))) as pbar:
        for i in pbar:
            current_row = raw_df.iloc[i]
            ...
            # Construct image paths
            current_img_name = f"{G_buffer_prefix}_{current_row['frame_idx']}.png"
            previous_img_name = f"{G_buffer_prefix}_{previous_row['frame_idx']}.png"
...
def images_to_video(image_paths, output_path, fps=30):
    """
    將圖片序列轉成 MP4 影片
    :param image_paths: List[str]，圖片路徑列表 (需照順序)
    :param output_path: str，輸出影片路徑，例如 "output.mp4"
    :param fps: int，每秒幀數 (frames per second)
    """
    ...
    # 建立 MP4 編碼器 (H.264 或 MP4V)
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")  # 若播放有問題可改 "avc1"
...
if __name__ == "__main__":
    ...
    TO_VIDEO = True                        # Combine images to video
    ...
    if REMOVE_IDENTICAL:
        for cfg in cfg_list:
            ...
            _ = G_Buffer(raw_df, "VFXColor")
            _ = G_Buffer(raw_df, "ShadowMask")
    ...
    if TO_VIDEO:
        for cfg in cfg_list:
            raw_df = pd.read_csv(f"./data/{cfg.record_name}/{cfg.mode_name}_frame_index.csv")

            row = raw_df.iloc[0]
            root_path = os.path.join(ROOT_DIR, row["record"], row["mode"])
            images_path_list = [f"{root_path}/colorNoScreenUI_{idx}.png" for idx in raw_df["frame_idx"]]

            video_output_path = f"{os.path.dirname(root_path)}/fps_{cfg.fps}.mp4"
            images_to_video(images_path_list, video_output_path, cfg.fps)
    ...
    if CHECK_IDENTICAL_CROSS_FPS:
        for cfg in cfg_list:
            ...
            fps_30_df = pd.read_csv(f"./data/{cfg.record_name}/{cfg.mode_name}_frame_index.csv", dtype={"reason": "string"})
            fps_60_df = pd.read_csv(f"./data/{cfg.record_name}/{fps_60_name}_frame_index.csv", dtype={"reason": "string"})

            print(f"Checking identical frames between fps 30 and fps 60 for record: {cfg.record_name}, mode: {cfg.mode_name}")
