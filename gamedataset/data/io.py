...
def load_backward_velocity_tensor(exr_path):
    exr_data = loadEXR(exr_path)
    height, width, _ = exr_data.shape
    motion_1_to_0 = np.stack([exr_data[..., 2], exr_data[..., 1]], axis=-1)
    motion_1_to_0[..., 0] = -1 * width * motion_1_to_0[..., 0]
    ...
    while nan_checker(motion_1_to_0):
        ...
        failed_counter += 1
        if failed_counter > 5:
            raise ValueError(f"Failed to load valid EXR data from {exr_path} due to NaN values.")

    backward_velocity = torch.from_numpy(motion_1_to_0).permute(2, 0, 1).unsqueeze(0).float().cuda()
    depth_0 = torch.from_numpy(exr_data[..., 0]).unsqueeze(0).unsqueeze(0).float().cuda()
    return backward_velocity, depth_0


def load_forward_velocity_tensor(exr_path):
    exr_data = loadEXR(exr_path)
    height, width, _ = exr_data.shape
    motion_1_to_0 = np.stack([exr_data[..., 2], exr_data[..., 1]], axis=-1)
    motion_1_to_0[..., 0] = -1 * width * motion_1_to_0[..., 0]
    ...
    forward_velocity = torch.from_numpy(motion_1_to_0).permute(2, 0, 1).unsqueeze(0).float().cuda()
    depth_0 = torch.from_numpy(exr_data[..., 0]).unsqueeze(0).unsqueeze(0).float().cuda()
