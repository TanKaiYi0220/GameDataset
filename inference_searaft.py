...
def create_color_bar_with_labels(colormap, vmin, vmax, width=50, height=400, n_ticks=5):
    """建立有實際 EPE 標籤的 colorbar（使用 matplotlib）"""
    gradient = np.linspace(1, 0, height).reshape((height, 1))
    gradient = np.repeat(gradient, width, axis=1)
    cmap = plt.get_cmap('jet') if isinstance(colormap, int) else plt.get_cmap(colormap)
    ...
    for i in range(n_ticks):
        y = int(i * height / (n_ticks - 1))
        val = vmax - i * (vmax - vmin) / (n_ticks - 1)
        label = f"{val:.2f}"
        cv2.putText(bar_bgr, label, (width + 5, y + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
    return bar_bgr
